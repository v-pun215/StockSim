...
func RegisterPortfolioHandlers(mux *http.ServeMux) {
...
type Holding struct {
	...
	DailyChange    float64 `json:"daily_change"`    // price delta from prior close
	DailyChangePL  float64 `json:"daily_change_pl"` // shares * daily change
	AllocationPct  float64 `json:"allocation_pct"`  // share of portfolio market value
	PrevClose      float64 `json:"prev_close"`      // prior close used for daily calc
}
...
type PortfolioSummary struct {
	...
	TotalGainSincePrev float64 `json:"total_gain_since_prev"` // networth - previousNetworth
	TotalGainPct       float64 `json:"total_gain_pct"`
	Diversification    int     `json:"diversification"` // number of unique stocks
	LeaderPosition     int     `json:"leaderboard_position,omitempty"`
	LeaderTotal        int     `json:"leaderboard_total,omitempty"`
	LastUpdated        string  `json:"last_updated"` // local time RFC3339
}

type TransactionOut struct {
	ID        int64   `json:"id"`
	Timestamp string  `json:"timestamp"` // local formatted
	StockID   string  `json:"stock_id"`
...
func portfolioHandler(w http.ResponseWriter, r *http.Request) {
	...
	// compute allocation % per holding (use totalMarketValue; if zero avoid div-by-zero)
	...
	// compute networth and previousNetworth (using prevClose)
	...
	// compute diversification (count of unique stocks)
	diversification := len(holdings)

	// optionally compute leaderboard position (small O(N) loop over users)
	...
	// sort holdings by market value desc for UI niceness
	sort.Slice(holdings, func(i, j int) bool {
		return holdings[i].MarketValue > holdings[j].MarketValue
	})

	// add name lookup (optional) using stocks list to populate Name
	stocksLock.Lock()
	for i := range holdings {
...
func transactionsHandler(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		tstr := ""
		if ts.Valid {
			// try to parse common sqlite timestamp formats
...
func getPreviousClose(stockID string) (float64, error) {
	// compute local start of day
	nowLocal := time.Now().Local()
	startOfDayLocal := time.Date(nowLocal.Year(), nowLocal.Month(), nowLocal.Day(), 0, 0, 0, 0, nowLocal.Location())

	// query price_history for the most recent entry before startOfDayLocal
	// price_history.time column contains RFC3339 strings or sqlite default timestamps - we compare as text.
	...
	// fallback: try latest price_history regardless of time
...
func parseDBTimeToLocal(src string) time.Time {
	...
	// try sqlite default "2006-01-02 15:04:05"
